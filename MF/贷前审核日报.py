...
def getdata(i,j,table_name):
    ...
    from \
    ( SELECT date(created_time) as day_key,id FROM bmdb.tb_credit_record\
    WHERE DATE(created_time) >= date('%s') and  DATE(created_time) <=  date('%s') \
    AND task_status in ('%s','%s') group by date(created_time),id\
    )t1 left join\
    ( SELECT  cr.id,DATE(cr.created_time) as day_key FROM bmdb.tb_credit_record  cr, bmdb.tb_score_rule_result sr \
